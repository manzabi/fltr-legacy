...
class Slider extends Component {
    ...
    componentDidMount () {
        if (this.props.collapseHeight && this.props.colapseHeightUpdate) {
            const root = this;
            window.refresing = setInterval(function () {
                if (root.refs.slider.clientHeight !== root.props.collapseHeight) {
                    root.props.colapseHeightUpdate(root.refs.slider.clientHeight);
                }
            }, 1000);
        }
    }
    ...
    componentWillUnmount () {
        if (window.refresing) window.clearInterval(window.refresing);
    }
    toggleMenu () {
    ...
    render () {
        return (
            <div className='top-slider' ref='slider'>
            <div className='slider-container' style={{backgroundImage: `url(${this.props.backgroundImage})`}}>
            ...
            <a className='unstyled-link btn btn-small btn-blue' onClick={this.redirectToLogin} target='_top'>Log in</a>
            <a href='javascript:void(0);' className='menu-toggle' onClick={this.toggleMenu}>&#9776;</a>
            </div>
            <div id='default-slider'>
            {this.props.children}
            </div>
            </div>
            </div>
    ...
    Slider.propTypes = {
        ...
        colapseHeightUpdate: PropTypes.func,
